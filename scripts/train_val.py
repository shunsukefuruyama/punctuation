...
class DistilBERT_Arch(nn.Module):
    def __init__(self, distilbert):
        ...
        self.relu = nn.ReLU()
        self.fc1 = nn.Linear(768, 1568)
        self.fc2 = nn.Linear(1568, 4)
        self.softmax = nn.LogSoftmax(dim=1)
    def forward(self, input_ids, mask):        
        cls_hs = self.distilbert(input_ids, attention_mask=mask)[0]
...
############################################
#giving class_weights a further adjustment
############################################
class_weights = compute_class_weight('balanced', np.unique(punc_cat), punc_cat.numpy()) * np.array([1, 2, 5, 2])
